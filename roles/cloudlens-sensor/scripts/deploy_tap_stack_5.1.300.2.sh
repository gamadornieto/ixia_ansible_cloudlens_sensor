...
#docker rm sensor
    ...
    docker run --name 'sensor' --restart on-failure -d --net=host --device=/dev/mem --cap-add=SYS_RAWIO --cap-add=SYS_RESOURCE --cap-add=SYS_ADMIN --cap-add=NET_ADMIN --cap-add=NET_RAW -e REGISTRATION_URL=${REGISTRATION_URL} -e DRIVERLESS_MODE=${DRIVERLESS_MODE} -e ALLOW_L2_IFACE=${ALLOW_L2_IFACE} -v /var/cloudtap:/var/cloudtap ${DOCKER_LOCAL_REGISTRY}:5000/sensor:${VERSION}
}

function do_uninstall() {
    sensor_id=`docker ps -a -q -f name=sensor`
    if [ "x$sensor_id" != "x" ]; then
...
function do_cleanup_tunnels() {
    #checking if active tunnels exists
    tunnels=`ip link | awk -F': ' '$0 ~ "gre[0-9]+[0-9]+@NONE"{print $2;getline}' | sed 's/@NONE//'`
    echo -e "\nFollowing links will be removed:\n$tunnels"
    for link in ${tunnels}; do
        sudo ip link delete $link
    done
    echo -e "\nAll dependent tunnels are removed"
}
...
function do_upgrade() {
    ...
    sudo cp -r /var/cloudtap /var/cloudtap.upgrade
    do_uninstall
    do_cleanup_tunnels
    sudo mv /var/cloudtap.upgrade /var/cloudtap
    do_install $1 $2
}
